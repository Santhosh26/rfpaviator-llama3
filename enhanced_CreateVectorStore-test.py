...
def main():
    if len(sys.argv) > 1:
        ...
        root_directory_path = root_directory_path.strip('"').strip("'")
            
        # logger.info(f"Processing root directory: {root_directory_path}")
    else:
...
def process_files(directory_path, files_in_directory):
    ...
    for file_type, loader_class in [('.pdf', PDFMinerLoader), ('.docx', Docx2txtLoader), ('.xlsx', UnstructuredExcelLoader), ('.txt', TextLoader)]:
        if any(file.endswith(file_type) for file in files_in_directory):
            ...
            Chroma.from_documents(documents=clean_chunks, embedding=embeddings, persist_directory=persist_dir)
